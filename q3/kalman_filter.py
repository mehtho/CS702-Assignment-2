...
class KalmanFilter:
    ...
    def filter(self, data) -> np.ndarray:
        """Predict and update the state estimate."""
        x_hat = np.array([data['x_px'][0], 0, data['y_px'][0], 0]).reshape(4, 1)
        x_hat += mvn.rvs(mean=np.zeros(4), cov=self.P, size=1).reshape(4, 1)
        x_meas = np.zeros((len(data) + 1, self.N, 4))
        x_meas[0, :, :] = mvn.rvs(mean=x_hat.reshape(4, ), cov=self.P, size=self.N)
        for i in range(len(data)):
            # Prediction
            x_hat = self.predict(x_hat)
            
            # Update if there is a measurement
            if not np.isnan(data['x_px'][i]):
                z = np.array([data['x_px'][i], data['y_px'][i]]).reshape(2, 1)
                x_hat = self.update(x_hat, z)
            
            x_meas[i + 1, :, :] = mvn.rvs(mean=x_hat.reshape(4, ), cov=self.P, size=self.N)
        return x_meas
